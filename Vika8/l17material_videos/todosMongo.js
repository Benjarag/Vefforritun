...
let nextId = 11;
let users = [{ id: 5, username: "Alice", age: 25 }, { id: 10, username: "Bob", age: 30 }];
let notes = [{ userId: 5, userNotes: [{ id: 10, name: "todos for today", content: "Prepare Lab 6" }, { id: 12, name: "memo for l15", content: "Do not forget to mention Heroku" }] },
{ userId: 10, userNotes: [{ id: 1, name: "shopping list", content: "Milk, Cheese" }] }];
...
app.get('/users/:userId', (req, res) => {
    ...
    db.collection("users").findOne({ _id: id }, function (err, user) {
        if (err || user === null) {
            return res.status(404).json({ 'message': "User with id " + req.params.userId + " does not exist." });
        } else {
...
app.post('/users', (req, res) => {
    if (req.body === undefined || req.body.username === undefined || req.body.age === undefined) {
        res.status(400).json({ 'message': "username and age fields are required in the request body" });
    } else {
...
app.put('/users/:userId', (req, res) => {
    if (req.body === undefined || req.body.username === undefined || req.body.age === undefined) {
        ...
        let id = new mongo.ObjectID(req.params.userId);
        let newValues = { $set: {username: req.body.username, age: req.body.age } };
...
app.patch('/users/:userId', (req, res) => {
    if (req.body === undefined || (req.body.username === undefined && req.body.age === undefined)) {
        res.status(400).json({ 'message': "Either username or age field is required in the request body" });
    } else {
    ...
    db.collection("users").find({}).toArray(function (err, users) {
        db.collection("users").deleteMany({}, function (err, obj) {
            if (err || obj === null || obj.result.ok !== 1) {
                res.status(500).json({ 'message': "Deletion failed with error: " + err });
            } else {
                res.status(200).json(users);
            }
        });
...
app.get('/users/:userId/notes/:noteId', (req, res) => {
    for (let i = 0; i < notes.length; i++) {
        if (notes[i].userId == req.params.userId) {
            ...
            res.status(404).json({ 'message': "Note with id " + req.params.noteId + " does not exist for user " + req.params.userId });
            return;
        }
    }
    res.status(404).json({ 'message': "User with id " + req.params.userId + " does not exist" });
...
app.put('/users/:userId/notes/:noteId', (req, res) => {
    res.status(200).json({ 'message': "Update note with id " + req.params.noteId + " for user with id " + req.params.userId });
});
